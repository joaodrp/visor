...
task :lines do
  system "find #{image+image_spec} #{meta+meta_spec} #{common+common_spec} #{auth+auth_spec} -name '*.rb' | xargs wc -l"
end

desc "Generate YARD documentation"
task :ydoc do
  output = "~/workspace/gh-pages/visor/"
  system "yardoc #{image} #{meta} #{auth} #{common} -o #{output} --protected -b #{output}.yardoc --title VISoR --main README.md - #{files}"
